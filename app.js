...
function store(){
 ...
 <div class="account-status"><span>${currentAccount.discord?.tag?`<span class="status-dot"></span>${escapeHtml(currentAccount.discord.tag)}`:"Discord not linked"}</span>${currentAccount.discord?'<button class="button" id="store-unlink-discord">Unlink</button>':'<span style="font-size:12px;color:#718091">Use Discord linking</span>'}</div>
...
function accountPage(){
 ...
 ${currentAccount.discord?'<button class="button" id="unlink-discord">Unlink Discord</button>':'<p style="color:#8995a5;font-size:13px;line-height:1.6">Discord linking is managed through the BlockBound Discord verification flow.</p>'}</div></div>
